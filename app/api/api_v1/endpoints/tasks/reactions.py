...
async def add_reaction(
    ...
    task = Task.manager(db).get(id=reaction.task_id)
    for r in task.reactions:
        if reaction.emoji == r.emoji:
            UserReaction.manager(db).create(user_id=user.id, reaction_id=r.id)
            db.commit()
            db.refresh(task)
            return Task.manager(db).get(id=reaction.task_id)

    reaction = Reaction.manager(db).create(
        emoji=reaction.emoji,
        task_id=reaction.task_id
    )
    UserReaction.manager(db).create(user_id=user.id, reaction_id=reaction.id)
    db.commit()
    db.refresh(task)
    return Task.manager(db).get(id=reaction.task_id)
...
async def remove_reaction(
    ...
    for r in task.reactions:
        if reaction.emoji == r.emoji:
            try:
                user_reaction = UserReaction.manager(db).get(
                    reaction_id=r.id,
                    user_id=user.id
                )
            ...
            UserReaction.manager(db).delete(user_reaction)
            if len(r.users) == 0:
                Reaction.manager(db).delete(r)
            return Task.manager(db).get(id=reaction.task_id)
    return HTTPException(status_code=404, detail="No reactions according to user was found.")
