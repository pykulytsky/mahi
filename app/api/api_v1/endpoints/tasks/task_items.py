...
async def create_task(
    ...
    instance = Task.manager().create(**dict(task_in))
...
async def get_tasks_by_date(
    ...
    owned_projects = Project.manager().filter(owner=user)
    tasks = []
    for project in owned_projects:
        tasks += Task.manager().filter(deadline=raw_date, project=project)
    return tasks
...
async def move_task_to_proejct(
    ...
    task = Task.manager().get(id=id)
    if task.project.owner == user:
        updated_task = Task.manager().update(id=task.id, project_id=project_id)
        return updated_task
    else:
...
async def reorder_tasks(
    ...
    destination = model.manager().get(id=reorder_schema.destination_id)

    return Task.manager().reorder(instance, destination, reorder_schema.order)
