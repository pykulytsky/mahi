...
def test_project_participants_have_permissions_for_included_tasks(task, project, another_user, db):
...
def test_project_participants_have_permissions_for_section_included_tasks(project, another_user, db, section_task):
    project.participants.append(another_user)
    db.add(project)
    db.commit()
    db.refresh(project)
